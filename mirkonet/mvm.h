...
#define OP5_LOG       0x19
#define OP5_EXT       0x1A
#define OP5_XCALL     0x1B
#define OP5_GPIO      0x1C


#define EXT_ISZERO    0
#define EXT_MLOAD     1
#define EXT_MSTORE    2
#define EXT_ADDRESS   3
#define EXT_ORIGIN    4
#define EXT_NUMBER    5
#define EXT_TIMESTAMP 6
#define EXT_GASLEFT   7
...
#define BYTE_LOG(n)    ENCODE(OP5_LOG, (n))
...
#define BYTE_DCALL     ENCODE(OP5_XCALL, 1)
#define BYTE_GPIOWRITE ENCODE(OP5_GPIO, 0)
#define BYTE_GPIOREAD  ENCODE(OP5_GPIO, 1)
#define BYTE_GPIOMODE  ENCODE(OP5_GPIO, 2)
#define BYTE_ADCREAD   ENCODE(OP5_GPIO, 3)
#define BYTE_DACWRITE  ENCODE(OP5_GPIO, 4)
...
struct Contract {
    ...
    bool cacheCode(const uint8_t* bytecode, uint16_t len, bool temporary = false) {
        if (!bytecode || len == 0 || len > MVM_MAX_CODE) return false;
    ...
    Hash256 stateHash() const {
        uint8_t buf[MVM_MAX_STORAGE * 8 + 128];
        size_t off = 0;
        memcpy(buf + off, name, 16); off += 16;
        memcpy(buf + off, addr.bytes, 4); off += 4;
        memcpy(buf + off, deployer.id, 6); off += 6;
        ...
        uint8_t sidx[MVM_MAX_STORAGE];
        uint8_t sn = 0;
        for (int i = 0; i < MVM_MAX_STORAGE; i++)
            if (storage[i].used) sidx[sn++] = i;
        for (uint8_t a = 1; a < sn; a++) {
            uint8_t key = sidx[a];
            int b = a - 1;
            while (b >= 0 && storage[sidx[b]].key > storage[key].key) {
                sidx[b + 1] = sidx[b]; b--;
            }
            sidx[b + 1] = key;
        }
        for (uint8_t i = 0; i < sn; i++) {
            memcpy(buf + off, &storage[sidx[i]].key, 4); off += 4;
            memcpy(buf + off, &storage[sidx[i]].value, 4); off += 4;
        }
    ...
    MVMStatus execute(Contract& contract, const MVMContext& ctx,
        ...
        while (_status == VM_RUNNING) {
            ...
            case OP5_XCALL: {
                ...
                subCtx.localNode = ctx.localNode;
                subCtx.localNodeSet = ctx.localNodeSet;
                subCtx.allowHardwareEffects = ctx.allowHardwareEffects;
            ...
            case OP5_GPIO: {
                ...
                bool hwAllowed = ctx.allowHardwareEffects &&
                                 ctx.localNodeSet &&
                                 contract.host == ctx.localNode &&
                                 g_gpioCallback != nullptr;
                if (mod == 0) {
                    if (_sp < 2) { _status = VM_ERR_STACK_UNDER; break; }
                    ...
                    if (hwAllowed) g_gpioCallback(0, (uint8_t)pin, val);
                } else if (mod == 1) {
                    if (_sp == 0) { _status = VM_ERR_STACK_UNDER; break; }
                    ...
                    uint32_t val = 0;
                    if (hwAllowed) val = g_gpioCallback(1, (uint8_t)pin, 0);
                    _stack[_sp-1] = val;
                } else if (mod == 2) {
                    if (_sp < 2) { _status = VM_ERR_STACK_UNDER; break; }
                    ...
                    if (hwAllowed) g_gpioCallback(2, (uint8_t)pin, mode);
                } else if (mod == 3) {
                    if (_sp == 0) { _status = VM_ERR_STACK_UNDER; break; }
                    uint32_t pin = _stack[_sp-1];
                    if (!gpioIsAllowed((uint8_t)pin)) {
                    ...
                    uint32_t val = 0;
                    if (hwAllowed) val = g_gpioCallback(3, (uint8_t)pin, 0);
                    _stack[_sp-1] = val;
                } else if (mod == 4) {
                    if (_sp < 2) { _status = VM_ERR_STACK_UNDER; break; }
                    uint32_t val = _stack[--_sp];
                    ...
                    if (hwAllowed) g_gpioCallback(4, (uint8_t)pin, val & 0xFF);
                } else {
                    _status = VM_ERR_CODE_BOUNDS; break;
                }
                _pc++; break;
