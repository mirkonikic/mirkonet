...
struct Transaction {
    ...
    uint8_t  argCount;
    uint16_t deployCodeLen;
    Hash256  deployCodeHash;
    uint8_t  publicKey[64];
    uint8_t  signature[64];

    Hash256 signingHash() const {
        uint8_t buf[160];
        size_t off = 0;
        ...
        buf[off++] = argCount;
        for (uint8_t i = 0; i < argCount && i < MVM_MAX_ARGS; i++) {
            memcpy(buf + off, &args[i], 4); off += 4;
        }
        memcpy(buf + off, &deployCodeLen, 2); off += 2;
        memcpy(buf + off, deployCodeHash.bytes, HASH_SIZE); off += HASH_SIZE;
        return sha256(buf, off);
    }

    Hash256 hash() const {
        uint8_t buf[HASH_SIZE + 64 + 64];
        Hash256 sh = signingHash();
        memcpy(buf, sh.bytes, HASH_SIZE);
        memcpy(buf + HASH_SIZE, publicKey, 64);
        memcpy(buf + HASH_SIZE + 64, signature, 64);
        return sha256(buf, sizeof(buf));
    }

    bool hasSignature() const {
        for (int i = 0; i < 64; i++) if (signature[i]) return true;
        return false;
    }
};
...
struct Block {
    BlockHeader header;
    Hash256     blockHash;
    uint8_t     validatorPublicKey[64];
    uint8_t     validatorSignature[64];
    ...
    bool hasValidatorSignature() const {
        for (int i = 0; i < 64; i++) if (validatorSignature[i]) return true;
        return false;
    }
};
...
struct StakeInfo {
    ...
    bool     jailed;
    uint32_t jailUntilEpoch;
    uint32_t slashCount;
    uint32_t missedBlocks;
    uint32_t lastProducedBlock;
};
