    ...
    static Result assemble(const String& source) {
        ...
        while (pos <= (int)src.length()) {
            ...
            if      (mn == "HALT")      r.code[off++] = BYTE_HALT;
                ...
                ContractAddr ca = ContractAddr::fromName(operand.c_str());
                uint32_t h = ((uint32_t)ca.bytes[0] << 24) |
                             ((uint32_t)ca.bytes[1] << 16) |
                             ((uint32_t)ca.bytes[2] << 8)  |
                              ca.bytes[3];
                r.code[off++] = BYTE_PUSH32;
                r.code[off++] = (uint8_t)(h >> 24);
                r.code[off++] = (uint8_t)(h >> 16);
                r.code[off++] = (uint8_t)(h >> 8);
                r.code[off++] = (uint8_t)(h & 0xFF);
            }
