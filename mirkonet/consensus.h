...
public:
    ...
    void syncGenesisTime(uint32_t peerElapsedMs, uint32_t nowMs) {
        if (peerElapsedMs >= nowMs) return;
        uint32_t peerGenesis = nowMs - peerElapsedMs;
        if (peerGenesis < genesisTime) {
            Serial0.printf("[Genesis] Synced genesisTime %u -> %u (peer elapsed=%ums)\n",
                          genesisTime, peerGenesis, peerElapsedMs);
            genesisTime = peerGenesis;
        }
    }
